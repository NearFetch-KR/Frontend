...
function includeRouter(cb) {
  ...
  // 회원가입&로그인(기능)
     ...
     fetch("http://172.30.1.3:8000/users/signup", {
         method: "POST",
         headers: {
     ...
     .catch((error) => console.log("error:", error));
      ...
      fetch("http://172.30.1.3:8000/users/signin", {
          method: "POST",
          headers: {
              "Content-Type": "application/json",
          },
      ...
      logoutBtn.addEventListener('click', e => {
          ...
          fetch("http://172.30.1.3:8000/users/logout", {
              method: "POST",
              headers: {
                  "Content-Type": "application/json",
                  Authorization: token
  ...
  function mainRecomInit(){
      fetch("http://172.30.1.3:8000/products/list", {
          method: "GET",
          headers: {
              "Content-Type": "application/json",
          },
      ...
      .then((response) => { 
          ...
          for (let i = 0; i < randomRecomItem.length; i += 1) {
              ...
              // itemImg를 클릭하면 itemDetail파일에 뿌려주기
              const imgWrapAtag=document.querySelectorAll(".imgWrapAtag")   
              console.log(imgWrapAtag);
              

              imgWrapAtag.forEach((el,index) => {
                el.onclick = (e) => {
                    // e.preventDefault();
                    e.target.href=`http://127.0.0.1:5500/item%20detail/itemdetail.html`;
                    let parentTag=e.target.parentElement;  //a태그
                    let child=parentTag.childNodes; //이미지 태그
  ...
  function goCart(){
      ...
      .then((response) => {
          ...
          cartBtn.forEach((el,index) => {
              ...
              fetch("http://172.30.1.3:8000/users/cart", {
                  method: "POST",
                  headers: {
                      "Content-Type": "application/json",
  ...
  //     fetch(`http://172.30.1.3:8000/products/detail/${sku_number}`, {
  //         method: "GET",
  //         headers: {
  //             "Content-Type": "application/json",
  //         },
  ...
  //         fetch(`http://172.30.1.3:8000/products/list?large_category=${gender}&medium_category=${categoryMedium}&small_category=${categorySmall}`, {
  //             method: "GET",
  //             headers: {
  //                 "Content-Type": "application/json",
  //             },
...
//       })}
  ...
  saveAddressBtn.addEventListener('click', e => {
      ...
      let token = localStorage.getItem('login-token');      
      fetch("http://172.30.1.3:8000/users/register/location", {
          method: "POST",
          headers: {
              "Content-Type": "application/json",
              Authorization: token
