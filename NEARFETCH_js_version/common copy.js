  ...
  fetch("http://172.30.1.40:8000/products/list", {
    ...
    .then((response) => {
      ...
      for (let i = 0; i < randomRecomItem.length; i += 1) {
        ...
        div2.textContent = randomRecomItem[i].price
          .toString()
          .replace(/\B(?=(\d{3})+(?!\d))/g, ",");
...
itemBuyCart.addEventListener("click", (e) => {
  ...
  fetch("http://172.30.1.40:8000/users/cart", {
    ...
    .then(function () {
      // alert('장바구니 페이지로 이동하시겠습니까?');
      window.location.href = "http://127.0.0.1:5500/myinfoAll/add/cart.html";
    })
    .catch((error) => console.log("error:", error));
});
  ...
  fetch("http://172.30.1.40:8000/products/list", {
    ...
    .then((response) => {
      ...
      var sameBrandItemList = response.result.filter((item) => {
        if (
          item.itemBrand.includes(samBrand) &&
          item.gender.includes(sameGender) &&
          item.categoryMedium.includes(sameCategoryMedium)
        ) {
          return true;
        }
        return false;
      ...
      for (var i = 0; i < sameBrandItemList.length; i++) {
        const randomItem =
          sameBrandItemList[
            Math.floor(Math.random() * sameBrandItemList.length)
          ];
        if (!randomRecomItem.includes(randomItem)) {
          randomRecomItem.push(randomItem);
          if (randomRecomItem.length == 10) {
            //10개만 노출
            break;
          }
        } else {
